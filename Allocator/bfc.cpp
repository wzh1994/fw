...
namespace memory {
	...
	bool BfcAllocator::extend(size_t roundedBytes) {
		...
		while (currExtendBytes_ < roundedBytes &&
			currExtendBytes_ < kExtendThreshold) {
			currExtendBytes_ *= 2;
			increased = true;
		}
	...
	void BfcAllocator::shrink() {
		for (auto it = chunkRegister_.begin(); it != chunkRegister_.end(); ) {
			if (!it->inUse() && it->isMonoblock()) {
				FW_ASSERT(it->binIdx != kInvalidBinInedx);
	...
	void BfcAllocator::releaseRaw(void* ptr, size_t nbytes) {
		...
		FW_ASSERT(it != chunkRegister_.end());
		freeAndCoalesce(&*it);
	}
	...
	void* BfcAllocator::findChunkPtr(BinIndex binIdx, size_t roundedBytes) {
		for (; binIdx < kNumBins; ++binIdx) {
			...
			if (it != bin.freeChunks.end()) {
				...
				if (chunk->size >= roundedBytes * 2 ||
					chunk->size >= roundedBytes + kMaxInternalFragmentation) {
	...
	const BfcAllocator::Chunk* BfcAllocator::splitChunk(const Chunk* chunk,
		size_t num_bytes) {
		FW_ASSERT(!chunk->inUse());
		FW_ASSERT(chunk->binIdx == kInvalidBinInedx);
		FW_ASSERT(0 < num_bytes && num_bytes <= chunk->size);
		...
		FW_ASSERT(chunkRegister_.erase(*chunk));
		...
		c1->next = c2;
		c2->prev = c1;
		if ((c1->prev = prev)) prev->next = c1;
		if ((c2->next = next)) next->prev = c2;
	...
	void BfcAllocator::insertChunkIntoBin(const Chunk* chunk) {
		FW_ASSERT(!chunk->inUse() && (chunk->binIdx == kInvalidBinInedx));
		chunk->binIdx = binIdxForSize(chunk->size);
		FW_ASSERT(bins_[chunk->binIdx].freeChunks.insert(chunk).second);
	}

	void BfcAllocator::removeChunkFromBin(const Chunk* chunk) {
		FW_ASSERT(!chunk->inUse() && (chunk->binIdx != kInvalidBinInedx));
		FW_ASSERT(bins_[chunk->binIdx].freeChunks.erase(chunk));
	...
	const BfcAllocator::Chunk* BfcAllocator::merge(const Chunk* c1,
		const Chunk* c2) {
		FW_ASSERT(!c1->inUse() && !c2->inUse());
		FW_ASSERT(c1 == c2->prev && c2 == c1->next);
		const Chunk* prev = c1->prev;
		const Chunk* next = c2->next;
		void* const p = c1->ptr;
		size_t const s = c1->size + c2->size;
		FW_ASSERT(chunkRegister_.erase(*c1));
		FW_ASSERT(chunkRegister_.erase(*c2));

		auto res = chunkRegister_.emplace(p, s);
		FW_ASSERT(res.second);
		const Chunk* c3 = &*res.first;
		if ((c3->prev = prev)) prev->next = c3;
		if ((c3->next = next)) next->prev = c3;
