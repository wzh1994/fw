...
class BfcAllocator {
	friend BfcAllocator& cudaAllocator();
private:
	using BinIndex = int;
	...
	struct Chunk {
		...
		explicit Chunk(void* ptr) : ptr(ptr) {}
		Chunk(void* ptr, size_t size) : ptr(ptr), size(size) {}
...
public:
	...
	void release(void* ptr) {
		...
		FW_ASSERT(res != allocatedMap_.end());
...
inline BfcAllocator& cudaAllocator() {
