...
(cos_theta - 1.0f) < 1e-6) {
		...
		void crossAndAngle(float& r1, float& r2, float& r3, float& cos_theta,
			...
			cos_theta = (u1 * v1 + u2 * v2 + u3 * v3) / (
				std::sqrt(u1*u1 + u2 * u2 + u3 * u3) * std::sqrt(v1*v1 + v2 * v2 + v3 * v3));
		}
		...
		void calcLeftHalfBall(
			...
			float alpha, float colorScale, float sizeScale, uint32_t bufferStart, size_t nBalls) {
		...
		void calcCircularTruncatedCone(
			...
			calcCircularTruncatedConeGroup (
				...
				circle2, indices1, oddTrans, alpha, colorScale, sizeScale, bufferStart, nGroups, maxSize);
			calcCircularTruncatedConeGroup (
				dPointsIn, dSizesIn, dColorsIn, groupOffsets,
				bufferOffsets, indicesOffsets, buffer, dIndicesOut,
				circle1, indices2, evenTrans, alpha, colorScale, sizeScale, bufferStart, nGroups, maxSize);
			calcFinalIndices(
				groupOffsets, indicesOffsets, bufferOffsets, dIndicesOut, bufferStart, nGroups);
		}
}
	size_t pointToLine(
