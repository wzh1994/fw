...
uint32_t indices1[kCircleIndices]{
	0, 24, 1, 1, 24, 25,
	1, 25, 2, 2, 25, 26,
	2, 26, 3, 3, 26, 27,
	3, 27, 4, 4, 27, 28,
...
uint32_t indices2[kCircleIndices]{
	0, 24, 25, 1, 0, 25,
	1, 25, 26, 2, 1, 26,
	2, 26, 27, 3, 2, 27,
	3, 27, 28, 4, 3, 28,
...
float halfBallLeft[kHalfBallPoints * 3]{
#ifdef DRAW_BALL
	- 1.000, 0.000, 0.000,
	-0.966, 0.259, 0.000,
	-0.966, 0.129, 0.224,
...
float halfBallRight[kHalfBallPoints * 3]{
	0.000, 1.000, 0.000,
	0.000, 0.966, 0.259,
	0.000, 0.866, 0.500,
	0.000, 0.707, 0.707,
...
uint32_t halfBallIndicesLeft[kHalfBallIndices + 1]{
	0, 1, 2,
0, 2, 3,
0, 3, 4,
0, 4, 5,
...
uint32_t halfBallIndicesRight[kHalfBallIndices + 1]{
	0, 24, 1,
	1, 24, 25,
	1, 25, 2,
	2, 25, 26,
...
void calcOffsets(const size_t* groupOffsets,
	size_t* bufferOffsets, size_t* indicesOffsets, size_t size) {
	for (size_t idx = 0; idx < size; ++idx) {
...
void rotate(float u, float v, float w, float cos_theta,
	...
	if (std::abs(cos_theta - 1.0f) < 1e-6) {
		return;
	}
	normalize(u, v, w);
	float m[3][3];
...
void rotate(float u, float v, float w, float theta,
	...
	rotate(u, v, w, std::cos(theta), std::sin(theta), a, b, c);
}

void resizeAndTrans(float& x, float& y, float& z,
...
void crossAndAngle(float& r1, float& r2, float& r3, float& cos_theta,
	...
	cos_theta = (u1 * v1 + u2 * v2 + u3 * v3) / (
		std::sqrt(u1*u1 + u2 * u2 + u3 * u3) *
		std::sqrt(v1*v1 + v2 * v2 + v3 * v3));
}
...
void calcHalfBallItem(float* pBufferBase, float* halfBall,
...
void fillHalfBallIndices(size_t indexOffset,
...
void calcLeftHalfBall(
		...
		size_t nBalls) {
	for (size_t idx = 0; idx < nBalls; ++idx) {
		...
		calcHalfBallItem(pBufferBase, halfBallLeft, pos[0], pos[1],
			pos[2], size * sizeScale, color[0], color[1], color[2],
			normX, normY, normZ, alpha, colorScale);
		fillHalfBallIndices(
			indexOffset, pIndicesBase, halfBallIndicesLeft, bufferStart);
	}
}
	
void calcRightHalfBall(
		const float* dPointsIn, const float* dSizesIn, const float* dColorsIn,
		const size_t* groupOffsets, const size_t* bufferOffsets,
		const size_t* indicesOffsets, float* buffer, uint32_t* dIndicesOut,
		float alpha, float colorScale, float sizeScale, uint32_t bufferStart,
	...
	for (size_t idx = 0; idx < nBalls; ++idx) {
		...
		calcHalfBallItem(pBufferBase, halfBallRight, pos[0], pos[1], pos[2],
			size * sizeScale, color[0], color[1], color[2],
			normX, normY, normZ, alpha, colorScale);
		fillHalfBallIndices(
...
void calcHalfBall(const float* dPointsIn, const float* dSizesIn,
		const float* dColorsIn, const size_t* groupOffsets,
		size_t nGroups, const size_t* bufferOffsets,
		const size_t* indicesOffsets, float* buffer,
		uint32_t* dIndicesOut, float alpha, float colorScale,
		float sizeScale, uint32_t bufferStart) {

	// leftBall
	calcLeftHalfBall(dPointsIn, dSizesIn, dColorsIn, groupOffsets,
		bufferOffsets, indicesOffsets, buffer, dIndicesOut,
		alpha, colorScale, sizeScale, bufferStart, nGroups);
	// rightBall
	calcRightHalfBall(dPointsIn, dSizesIn, dColorsIn, groupOffsets,
		bufferOffsets, indicesOffsets, buffer, dIndicesOut,
		alpha, colorScale, sizeScale, bufferStart, nGroups);
}

void calcCircularTruncatedConeItem(float* pBufferBase,
	...
	for (size_t i = 0; i < kCirclePoints; ++i) {
		...
		rotate(axisX, axisY, axisZ, cos_theta, sin_theta,
			pBufferBase[7 * i], pBufferBase[7 * i + 1],
			pBufferBase[7 * i + 2]);
		resizeAndTrans(
			pBufferBase[7 * i], pBufferBase[7 * i + 1],
			pBufferBase[7 * i + 2], size, x, y, z);
		uint32_t baseIndex =
			bufferOffset - kCirclePoints + bufferStart;
...
void calcCircularTruncatedConeGroup(
		const float* dPointsIn, const float* dSizesIn,
		const float* dColorsIn, const size_t* groupOffsets,
		const size_t* bufferOffsets, const size_t* indicesOffsets,
		float* buffer, uint32_t* dIndicesOut, const float* circle,
		const uint32_t* indices, trans_func_t trans, float alpha,
		float colorScale, float sizeScale, uint32_t bufferStart,
		size_t nGroups, size_t totalLen) {
	for (size_t bidx =0; bidx < nGroups;++bidx){
		for (size_t j = 0; j < totalLen; ++j) {
			size_t idx = trans(j);
					
			size_t totalNum =
				groupOffsets[bidx + 1] - groupOffsets[bidx];
			if ((idx + 1) < totalNum) {
				size_t bufferOffset =
					bufferOffsets[bidx] / 7 +
					(kHalfBallPoints + kCirclePoints * (idx - 1));
				float* pBufferBase = buffer + bufferOffset * 7;
				uint32_t* pIndicesBase =
					dIndicesOut + indicesOffsets[bidx] +
					kHalfBallIndices + kCircleIndices * (idx - 1);
				float size = dSizesIn[groupOffsets[bidx] + idx];
				const float* color =
					dColorsIn + (groupOffsets[bidx] + idx) * 3;
				const float* pos =
					dPointsIn + (groupOffsets[bidx] + idx) * 3;
...
size_t oddTrans(size_t x) {
	return x * 2 + 1;
}

size_t evenTrans(size_t x) {
	return (x + 1) * 2;
}
...
void calcCircularTruncatedCone(
	...
	buffer[0] = 0;
	dIndicesOut[0] = 0;
	calcCircularTruncatedConeGroup (
		dPointsIn, dSizesIn, dColorsIn, groupOffsets,
		bufferOffsets, indicesOffsets, buffer, dIndicesOut,
		circle2, indices1, oddTrans, alpha, colorScale,
		sizeScale, bufferStart, nGroups, maxSize);
	calcCircularTruncatedConeGroup (
		dPointsIn, dSizesIn, dColorsIn, groupOffsets,
		bufferOffsets, indicesOffsets, buffer, dIndicesOut,
		circle1, indices2, evenTrans, alpha, colorScale,
		sizeScale, bufferStart, nGroups, maxSize);
	calcFinalIndices(groupOffsets, indicesOffsets,
		bufferOffsets, dIndicesOut, bufferStart, nGroups);
}
}
size_t pointToLine(
	...
	cudaMallocAlign(&bufferOffsets, (nGroups + 1) * sizeof(size_t));
	cudaMallocAlign(&indicesOffsets, (nGroups + 1) * sizeof(size_t));

	calcOffsets(dGroupOffsets,
		bufferOffsets, indicesOffsets, nGroups + 1);

	size_t totalBuffers = bufferOffsets[nGroups];
	size_t totalIndices = indicesOffsets[nGroups];

	cudaMallocAlign(&buffer, 2 * totalBuffers * sizeof(float));
	cudaMallocAlign(&indicesOut, 2 * totalIndices * sizeof(uint32_t));

	calcHalfBall(dPointsIn, dSizesIn, dColorsIn, dGroupOffsets,
		nGroups, bufferOffsets, indicesOffsets, buffer, indicesOut,
	...
	calcHalfBall(dPointsIn, dSizesIn, dColorsIn, dGroupOffsets,
		nGroups, bufferOffsets, indicesOffsets, buffer + totalBuffers,
		indicesOut + totalIndices, outterAlpha, 0.0f, 1.0f, totalBuffers / 7);
	calcCircularTruncatedCone(
		dPointsIn, dSizesIn, dColorsIn, dGroupOffsets, maxSizePerGroup,
		nGroups, bufferOffsets, indicesOffsets, buffer + totalBuffers,
		indicesOut + totalIndices, outterAlpha, 0.0f, 1.0f, totalBuffers / 7);

	cudaMemcpy(dBuffer, buffer,
		2 * totalBuffers * sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpy(dIndicesOut, indicesOut,
		2 * totalIndices * sizeof(float), cudaMemcpyHostToDevice);
	cudaFreeAll(bufferOffsets, indicesOffsets, buffer, indicesOut);
